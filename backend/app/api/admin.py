...
async def get_overview_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        # Total users
        total_users = await db.scalar(select(func.count(Student.id))) or 0
        ...
        return OverviewStats(
            ...
            active_users_month=0, new_users_today=0, new_users_week=0,
            new_users_month=0, total_lessons=0, total_messages=0,
            avg_lessons_per_user=0, avg_streak_days=0
        )
...
async def get_all_users(
    ...
    try:
        ...
        query = select(Student).options(selectinload(Student.current_level))
...
async def get_churned_users(
    ...
    try:
        ...
        logger.error(f"Error in get_churned_users: {e}")
        return []
...
async def get_usage_by_level(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error(f"Error in get_usage_by_level: {e}")
        return []
...
async def get_token_usage(
    ...
    try:
        ...
        for row in rows:
            ...
            name = row.first_name or "Usuario"
            if row.last_name:
        ...
        logger.error(f"Error in get_token_usage: {e}")
        return []
...
async def get_engagement_stats(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        logger.error(f"Error in get_engagement_stats: {e}")
        return EngagementStats(
            avg_session_duration_minutes=0, avg_messages_per_session=0,
            avg_daily_usage_minutes=0, retention_rate_7d=0,
            retention_rate_30d=0, churn_rate=0
        )
